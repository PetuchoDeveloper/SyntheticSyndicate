...
- **Single-domain task** → Spawn one worker, then the Technical Writer.
- **Multi-domain task** → Sequence workers in dependency order (contract-defining layer first), then the Technical Writer.
- **Every feature** ends with a Technical Writer pass. No exceptions.
- **Every feature** starts and ends with Git Specialist operations (branch setup and commit).
</worker_roster>
...
### Step 2: Context Gathering (Before Each Coding Agent)
...
- Provide the task description and the coding agent's `TASK_REPORT`
...
2. Create/update CLAUDE.md in every directory where code was added or modified
3. Produce TASK_REPORT-[task-id].md in .agents/coding_line/sprint-active/reports/
   - Summary of changes
...
└── reports/
    ├── TASK_REPORT-T-001.md        ← Coding agent report
...
## Feature Queue
| #  | Task   | Worker(s)          | Branch               | Code Status     | Docs Status     | Notes |
|----|--------|--------------------|----------------------|-----------------|-----------------|-------|
| 1  | T-001  | Backend            | feat/T-001-auth-api  | ✅ DONE         | ✅ DONE         |       |
| 2  | T-002  | Frontend           | feat/T-002-login-ui  | 🔄 IN PROGRESS  | ⏳ QUEUED       |       |
| 3  | T-003  | Mobile → Frontend  | feat/T-003-mobile    | ⏳ QUEUED       | ⏳ QUEUED       |       |

## Execution Notes
- [Timestamp] T-001 completed. Backend agent: 12 files modified, 3 created.
- [Timestamp] T-002 started. Context injected from app/(tabs)/CLAUDE.md, components/CLAUDE.md.
